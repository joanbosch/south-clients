...
export async function GET (request: Request, { params: { id } } : { params: { id: string } }) {
  try {
    ...
    const results: any = await executeQuery({
      query: `SELECT id as 'id', name as 'name', email as 'email', NIF as 'NIF', street as 'street', ZIP as 'ZIP', city as 'city', deposit as 'deposit' FROM evtybqup_southclientes.clients ${id ? `WHERE id = ${id}` : ''}`
    })
