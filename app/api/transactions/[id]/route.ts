...
export async function GET (request: Request, { params: { id } } : { params: { id: string } }) {
  try {
    ...
    const results: any = await executeQuery({
      query: `SELECT id as 'id', transactionDate as 'date', idClient as 'idClient', idPaymentMethod as 'idPaymentMethod' FROM evtybqup_southclientes.vtransactions ${id ? `WHERE id = ${id}` : ''}`
    })
